...
using (var tocDocument = PdfDocument.Load(CreatePdfWithToc(tocEntries)))
{
    ...
    for (var i = pagesCount - 1; i >= tocPagesCount; i--)
    {
...
var entryIndex = 0;
var entryPageIndex = tocPagesCount;

// Update TOC links and outlines so that they point to adequate pages instead of placeholder pages.
for (var i = 0; i < tocPagesCount; i++)
{
    foreach (PdfLinkAnnotation annotation in document.Pages[i].Annotations.OfType<PdfLinkAnnotation>())
    {
        PdfPage entryPage = document.Pages[entryPageIndex];
...
MemoryStream CreatePdfWithToc(List<(string Title, int PagesCount)> tocEntries)
{
    // Create new document.
    ...
    foreach ((var Title, var PagesCount) in tocEntries)
    {
        section.Blocks.Add(
            new Paragraph(document, Title)
